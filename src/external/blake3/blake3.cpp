...
static void compress(uint32_t* out, const uint32_t m[16], const uint32_t h[8], uint64_t t, uint32_t b, uint32_t d)
{
...
static void load(uint32_t d[16], const unsigned char s[64])
{
    uint32_t* end;

    for (end = d + 16; d < end; ++d, s += 4)
    {
        *d = (uint32_t)s[0] | (uint32_t)s[1] << 8 | (uint32_t)s[2] << 16 | (uint32_t)s[3] << 24;
    }
}

static void block(struct blake3* ctx, const unsigned char* buf)
{
...
void blake3_init(struct blake3* ctx)
{
    ctx->bytes = 0;
    ctx->block = 0;
    ctx->chunk = 0;
...
void blake3_update(struct blake3* ctx, const void* buf, size_t len)
{
...
void blake3_out(struct blake3* ctx, unsigned char* __restrict out, size_t len)
{
